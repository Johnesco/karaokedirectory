 ...
 * '' (so the default page carries no new markup at all); with it ON every
 * entry gets a line, dated or "Not verified". No DOM here — initFreshLens
 * guards its document access, which is what makes it callable from Node.
 */
...
describe('renderFreshness', () => {
  ...
  it('labels an unverified show, so a column is never blank', () => {
    initFreshLens(true);
    const html = renderFreshness(friday(), { now: NOW });
    assert.match(html, /venue-card__verified--never/);
  ...
  it("hangs the modifier on the caller's block and honours the tag", () => {
    initFreshLens(true);
    const html = renderFreshness(friday(), { block: 'kj-dossier', tag: 'div', now: NOW });
    assert.match(html, /^<div class="kj-dossier__verified kj-dossier__verified--never">/);
  });
});
