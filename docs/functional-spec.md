...
| 5 | A-Z full card bottom gap | `.venue-card--full`'s bottom inset reads 41px against a 24px `padding-bottom` at 1280px | A margin inside the last `.venue-detail__section` collapses out through it. #224 improved this from 65px to 41px via `.venue-card > :last-child`; the residue is inside the section and is tracked separately. | **Open** |
