...
- **Host** — "Presented by [host display]" for the **show being rendered**, resolved with `resolveHostFor(venue, scheduleEntry)`: the entry's own host if it has one, otherwise the venue-level host. The card read `venue.host` until #223, so a venue whose hosts all live on schedule entries showed no host line at all, and one with both a venue host and an override would have shown the wrong one
...
> **Presented By, in detail views.** `renderHostSection(venue, …)` takes the venue, not a host, and enumerates both scopes via `getVenueHosts()`. It took `venue.host` until #223, which left The Highball — no venue-level host, seven one-time shows each with its own host ref — with no block at all on the modal, the desktop pane, the map card and the A–Z full card, all four of which share `renderVenueDetailSections`. Hosts are deduplicated by display identity, since `hydrateVenues` builds a fresh object per entry from the registry refs. When a venue has more than one distinct host each block names the shows it covers, so several hosts never read as one host for everything; a lone host gets no attribution because it covers everything. Per-show `website` and `socials` now reach the page, which they never did through the schedule table's Host column (it renders `escapeHtml(formatHostDisplay(...))`, text only).
>
...
- **Display by surface:** none by default. With `?fresh=1` (§18 "Freshness lens", #259) the calendar cards, the detail schedule table on all four surfaces, and the KJ dossier show the date and its age.
...
That call was made in #210: `austin-deaf-club`'s stored copy was removed, so **no venue stores a derived tag today**. The reasoning is worth keeping — the stored and derived forms assert different things (*this venue is tagged Special Event* vs *this show is a one-off*), and they agreed only because that venue had a single one-time show. A recurring night would have badged its weekly card "Special Event". Removing it costs the venue's detail views that badge and leaves the calendar card untouched, since only the compact card injects.
...
- **Calendar cards** (§6) — a last line after the host: "✓ Verified Aug 28 · 6d", or "Not verified"
...
| 5 | Detail-surface trailing margin | The last `.venue-detail__section` keeps a full bottom margin on the modal (21px) and the desktop pane (24px), stacked on each surface's own padding | #224's `.venue-card > :last-child` rule reached the A-Z inline card but not the other three surfaces, which hang `.venue-detail` on a non-`.venue-card` wrapper. #226 added a matching `.venue-detail > :last-child` rule and zeroed the 4px `.venue-detail__host` paragraph margin that was collapsing out of the last section. All four surfaces measured flush. | **Resolved 2026-08** |
...
| 7 | Contrast gate covered only the day headers | `scripts/check-contrast.js` reported `14 pairs checked, all pass` while axe-core found 474 `color-contrast` violations on the live site | The script only inspects the seven day-header gradients — never body text, links or tags. Colours fixed in #229; widening the script to walk rendered text is tracked separately so the gate cannot go green over failures again. | **Partially resolved 2026-08** |
